...
class UserController extends Controller
{
  ...
  public function dashboard()
  {
    if (auth()->user()->verified_id != 'yes') {
      return redirect(url('settings/verify'));
    }
 ...
 /*My Changes $schedule*/
  public function profile($slug, $media = null, $schedule = null)
  {
    $isStagingApp = false;
    $media = request('media');
    $mediaTitle = null;
    ...
    // Stripe Key
      ...
      if (isset($media)) {
       /*My Changes ->where('updates.expired', 'no')*/
        $query = $user->media()->where('updates.expired', 'no');
      } else {
          /*My Changes ->where('updates.expired', 'no')*/
        $query = $user->updates()->where('updates.expired', 'no')->whereFixedPost('0');
      }
      ...
      /*My changes*/
      $query->when(request('query') == 'ajax-cat', function($q) {
           
           if(request('type') != 'all')
            $q->where('updates.cat_post', '=', request('type'));

  		}); 
      $query->when(request('type') == 'schedule', function($q) {
           
            $q->where('updates.is_schedule', '=', 'yes');
  			$q->where('updates.schedule_date_time', '>', date('Y-m-d H:i:00'));
  		});
  		
  		$query->when(request('type') != 'schedule', function($q) {
  		   // $q->where('updates.is_schedule', '<>', 'yes');
  			$q->where(function ($query1) {
                    $query1->where('updates.schedule_date_time', '<=', date('Y-m-d H:i:00'));
                    $query1->orWhereNull('updates.schedule_date_time');
                });
  		});
      ...
      $uri = request()->path();
      $uriCanonical = ($isStagingApp ? 'app/' : '').$user->username.$media;

      if ($uri != $uriCanonical) {
        $baseUrl = request()->getSchemeAndHttpHost().'/';

        return redirect()->away($baseUrl.$uriCanonical);
      }
      ...
      // Count all likes
      $likeCount = $user->likesCount() + $user->likesCountPro(); /*My Changes*/
      // Categories
      ...
      $plans = $user->plans()
        ...
        $userProducts->when(request('sort') == 'garments', function($q) {
          $q->where('type', 'garments');
        });
        
        // Filter by video Content
        $userProducts->when(request('sort') == 'video', function($q) {
          $q->where('type', 'video');
        });
        
         // Filter by snapchat Content
        $userProducts->when(request('sort') == 'snapchat', function($q) {
        ...
        $userProducts = $userProducts->orderBy('id', 'desc')
            ->paginate(15);
        /*My Changes*/
    $postCats = PostCategories::where('user_id',$user->id)->get();
    $user_status = DB::table('user_status')
         ->select(DB::raw('
        user_status.id id_noty,
        creator_status.title,
        creator_status.icon
        '))->leftjoin('creator_status', 'creator_status.id', '=', DB::raw('user_status.status_id'))
        ->where('user_status.user_id', '=',  $user->id)->where('creator_status.status', 'on')->get();
       //My Changes
    $ajax = trim($this->request->input('query'));

     
    if($ajax == 'ajax'){
        return view('users.profile-ajax',[
          'user' => $user,
            'updates' => $updates,
            ...
            'postCats' => $postCats, /*My Changes*/
            'user_status' => $user_status,
        ]);
    } elseif($ajax == 'ajax-schedule' || $ajax == 'ajax-cat') { 
        $countUp = $updates->count(); 
        $counterPosts = ($updates->total() - $this->settings->number_posts_show);
                $total = $updates->total();
                 return view('includes.updates', ['updates' => $updates, 'countUp' => $countUp, 'user' => $user,  'ajaxRequest' => true, 'counterPosts' => $counterPosts, 'total' => $total])->render();
    } else {
      return view('users.profile-new',[
          'user' => $user,
            'updates' => $updates,
            'findPostPinned' => $findPostPinned,
            '_stripe' => $_stripe,
  ...
  public function postDetail($slug, $id)
  {
    ...
    $user    = User::where( 'username','=', $slug )->where('status','active')->firstOrFail();
     /*My Changes*/
    $updates = Updates::whereUserId($user->id)->where('updates.expired', 'no')
    ...
    $updateCount = $updates->count();
      ...
      //<<<-- * Redirect the user real name * -->>>
      $uri = request()->path();
      $uriCanonical = ($isStagingApp ? 'app/' : '').$user->username.'/post/'.$updates[0]->id;
    ...
    public function uploadAvatar()
	  ...
	  $path = config('path.avatar');
	    ...
	    if ($this->request->hasFile('avatar')) {
				...
				$imgAvatar = $image->orientate()->fit(200, 200, function ($constraint) {
    ...
    public function settingsPage()
    {
      ...
      $categories = explode(',', auth()->user()->categories_id);
       /*My Changes*/
       $postCats = PostCategories::where('user_id',auth()->user()->id)->get();
	...
	Validator::extend('letters', function($attribute, $value, $parameters){
      ...
      if ($validator->fails()) {
       ...
       /*My Changes*/
         // Intro Video
       $fileuploaderFile = $this->request->input('fileuploader-list-intro_video');
      ...
      $user->website         = trim($this->request->website) ?? '';
       // My Changes here
      $user->website2         = trim($this->request->website2) ?? '';
      $user->website3         = trim($this->request->website3) ?? '';
	  // My Changes here end
     ...
     if ($fileuploaderFile) {
         ...
         //<<<-- Delete old image -->>>/
           Storage::delete(config('path.introvideo').auth()->user()->intro_video);
       }
       ...
       foreach ($fileuploaderFile as $key => $media) {
         ...
         User::whereId($user->id)->update([
           'intro_video' => $media['file']
         ]);

         // Move file to Storage
		$this->moveFileStorage($media['file'], $path);
		$file = Helper::getFile(config('path.introvideo').$media['file']);
     ...
     protected function moveFileStorage($file, $path)
 {
   ...
   $path = config('path.cover');
    ...
    public function like(Request $request){
        /*My changes*/
  		...
  		if(isset($request->is_shop)){
  		$like = Like::firstOrNew(['user_id' => auth()->id(), 'updates_id' => $request->id, 'is_shop' => $request->is_shop]);
  		...
  		if ($like->exists) {
            ...
            $notifications->update();
  			...
  			if ($user->user_id != auth()->id() && $user->user()->notify_liked_post == 'yes') {
  				/*My changes*/
  				Notifications::send($user->user_id, auth()->id(), $type, $request->id);
  			}
  		}
      ...
      return response()->json([
				'success' => true,
				'total' => isset($request->is_shop) ? $totalLikes : trans_choice('general.like_likes', $totalLikes, ['total' => $totalLikes]) /*My Changes*/
			]);
    ...
    public function savePrivacySecurity()
    {
      ...
      $user->show_my_birthdate = $this->request->show_my_birthdate ?? 'no';
       // My changes
      $user->subscribers_can_message = $this->request->subscribers_can_message ?? 'no';
      $user->non_subscribers_can_message_tip = $this->request->non_subscribers_can_message_tip ?? 'no';
      $user->tip_amount = $this->request->tip_amount ?? 0;
      $user->show_views = $this->request->show_views ?? 'no';
      // My changes End
    ...
    public function myPosts()
    {
      ...
      ->where('status', '<>', 'encode');
      // Sort by oldest
      $posts->when(request('sort') == 'active', function($q) {
        $q->where('status', 'active');
        
        $q->where('schedule_date_time', '<=', date('Y-m-d H:i:00'));
        $q->orWhereNull('schedule_date_time');
      });

      // Sort by pending
      $posts->when(request('sort') == 'schedule', function($q) {
        $q->where('is_schedule', 'yes');
        $q->where('schedule_date_time', '>', date('Y-m-d H:i:00'));
      });
   ...
   * @return Response
   */
	public function deleteIntro($id,$file)
	{
        ...
        Storage::delete(config('path.introvideo').$file);
