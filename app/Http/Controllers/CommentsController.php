...
class CommentsController extends Controller
{
   ...
   */
	 public function store(Request $request)
	 {
		...
		/*My changes*/
        if(isset($request->is_shop)){
	    $update = Products::where('id', $request->update_id)->first();
        } else {
	   ...
	   if (! isset($update)) {
		...
		if ($isReply) {
			...
			$sql->reply     = trim(Helper::checkTextDb($request->comment));
			$sql->updates_id = $request->update_id;
			$sql->is_shop = isset($request->is_shop) ? $request->is_shop : 'no'; /*My Changes*/
			$sql->user_id   = auth()->id();
			$sql->save();
		...
		if (auth()->id() != $update->user_id  && $update->user()->notify_commented_post == 'yes') {
			...
			/*My changes*/
			Notifications::send($update->user_id, auth()->user()->id, $type, $update->id);
		}
		...
		'idComment' => $idComment,
		'total' => !isset($request->is_shop) ? trans_choice('general.comment_comments', $totalComments, ['total' => $totalComments]) : $totalComments, /*My Changes*/
		'data' => ''.$wrapComments.'
