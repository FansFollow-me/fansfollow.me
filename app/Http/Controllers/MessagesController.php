...
class MessagesController extends Controller
{
  ...
  public function messages($id)
  {
    $user = User::whereId($id)->where('id', '<>', auth()->user()->id)->firstOrFail();
      ...
      //My changes
      $subscribedToYourContent = $user->subscribers_can_message == 'yes' ? $this->subscribedToYourContent($user) : true;

      $subscribedToMyContent = $user->subscribers_can_message == 'yes' ? $this->subscribedToMyContent($user) : true;
      //My changes end
...
protected function sendTipWallet()
   {
     ...
     $user = User::find($this->request->get('id_user'));
     $amount = $this->request->amount;
    
    if($amount == '' || $amount == NULL || $amount== 0){
        return response()->json([
         "success" => false,
         "errors" => ['error' => __('Please try again later!')]
       ]);
    }
     if (auth()->user()->wallet < Helper::amountGross($amount)) {
       return response()->json([
         ...
         .' '."<a ($settings->disable_wallet == 'off') ?? href='".url('my/wallet')."'  class='link-border'>".__('general.recharge') ."</a>" ]
       ]);
     }
     ...
     if (! $this->request->isLive) {
       Notifications::send($user->id, auth()->user()->id, '5', auth()->user()->id, $amount);
     }
       ...
       $conversation = Conversations::where('user_1', Auth::user()->id)
         ...
         $message->from_user_id    = Auth::user()->id;
         $message->to_user_id      = $user->id;
         $message->message         = trim(Helper::checkTextDb($this->request->get('message')));
         $message->updated_at      = now();
         $message->tip_chat           = 'yes';
    ...
    public function blockUser($id)
     {
      ...
      if ($verifyUser->isSuperAdmin()) {
        return redirect('messages/'.$verifyUser->id.'/'.$verifyUser->username);
      }

      $Blocked = BlockedUsers::firstOrNew(['blocked_by' => auth()->id(), 'user_id' => $id]);
