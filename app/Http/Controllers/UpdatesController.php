...
class UpdatesController extends Controller
{
  ...
  public function create()
  {
    ...
    if (! $fileuploader && ! $this->request->hasFile('zip')) {
      ...
      /*My Changes*/
     // $urlEmbed = Helper::cleanTextEmbed($input['description_']);
      //$checkVideoEmd = $urlEmbed ? true : false;
    ...
    $validator = Validator::make($input, [
     ...
     if ($validator->fails()) {
      ...
      //<===== Embed Content
      if ($this->request->is_embed) {
        $is_embed = 'yes';
        $description1 = $this->request->description;
        $description_ = $this->request->description_;
        $description = $description1.$description_;
      } else {
        $is_embed = 'no';
      }
      ...
      /*My Changes*/
      //<===== is_schedule
      if ($this->request->is_schedule) {
        $is_schedule = 'yes';
      } else {
        $is_schedule = 'no';
      }
      /*My Changes*/
      if ($this->request->schedule_date) {
        $schedule_date_time = Carbon::createFromTimestamp(strtotime(date('Y-m-d', strtotime($this->request->schedule_date)) . date('H:i', strtotime($this->request->schedule_time)) . ":00"));
      } else {
        $schedule_date_time = null;
      } 

      $post               = new Updates();
      $post->description  = trim(Helper::checkTextDb($description));
      ...
      $post->locked       = $locked;
      /*My Changes*/
      $post->is_story       = $is_story;
      $post->is_schedule       = $is_schedule;
      $post->schedule_date       = $this->request->schedule_date ? date('Y-m-d', strtotime($this->request->schedule_date)) : null;
      $post->schedule_time       = $this->request->schedule_time ? date('H:i', strtotime($this->request->schedule_time)) : null;
      $post->cat_post        = $this->request->cat_post;
      $post->schedule_date_time = $schedule_date_time;
      /*My Changes end*/
    ...
    /*My Changes*/
        if($post->cat_post){
       $data_cat = PostCategories::firstOrNew([
      'value' => $this->request->cat_post,
      'user_id' => auth()->id()
    ...
    if ($data_cat->exists ) {
      ...
      $data_cat->value = $this->request->cat_post;
      $data_cat->save();
    }
        }
      // Insert Files
     ...
     /*My Changes*/
      if ($videoUrl || $checkVideoEmd) {
        ...
        Media::create([
          ...
          'video' => '',
           /*My Changes*/
          'is_embed' => $is_embed,
          'video_embed' => ($videoUrl ? $urlVideo : ($checkVideoEmd ? $urlEmbed : '')),
          'music' => '',
          'file' => '',
          'file_name' => '',
          'file_size' => '',
      ...
      if ($videos->count() && $this->settings->video_encoding == 'on') {
        ...
        try {
          ...
          return response()->json([
            ...
            'pending' => true,
            'encode' => true,
            'schedule' => $is_schedule== 'yes' ? true : false
          ]);
      ...
      /*My Changes*/
      $postCats = PostCategories::where('user_id',auth()->id())->get();
      return response()->json([
              'success' => true,
              'data' => view('includes.updates', [
                'updates' => Updates::whereId($post->id)->where('updates.expired', 'no')
                ...
                /*My Changes */
              'post_tags' =>  view('includes.post-categories-tag', [
                'postCats' => $postCats,
                'url2' =>$this->request->url2,
                'user' => auth()->user()
                ])->render(),
                'dropcat' => view('includes.post-categories-drop', [
                'postCats' => $postCats,
                'user' => auth()->user()
                ])->render(), /*My Changes end*/
                'pending' => false,
  ...
  public function ajaxUpdates()
  {
    ...
    /*My Changes */
    $data = $query->where('updates.expired', 'no')
                ->where(function ($query) {
                    $query->where('updates.schedule_date_time', '<=', date('Y-m-d H:i:00'));
                    $query->orWhereNull('updates.schedule_date_time');
                })
  ...
  public function postEdit()
  {
    ...
    $input = $this->request->all();
    /*My changes*/
    $mediaFiles = $sql->media()->where('video_embed', '=', '')->orWhere('is_embed',  'no')->count();
  ...
  if ($mediaFiles == 0 && isset($getAllMedia) && $getAllMedia->video_embed != '' && ! $videoUrl
    ...
    if ($validator->fails()) {
     ...
     if ($videoUrl && ! $getAllMedia && ! $videoEmbed) {
      ...
      //My Changes
	    $isvideoEmbeded = $sql->media()->where('is_embed', '=', 'yes')->get();
		$issVideoEmbed = false;
		...
		if(!$issVideoEmbed)
			...
			$descr = Helper::linkText(Helper::checkText($sql->description, $isVideoEmbed ?? null));
			...
			$descr = Helper::linkText(Helper::checkTextEmbed(htmlspecialchars_decode($sql->description, null)));

			elseif(in_array(Helper::videoUrl($isVideoEmbed), array('youtube.com','www.youtube.com','youtu.be','www.youtu.be', 'm.youtube.com')))

			$descr = Helper::linkText(Helper::checkTextEmbedYou(htmlspecialchars_decode($sql->description, $isVideoEmbed ?? null)));

			}

      return response()->json([
              'success' => true,
/*My Changes*/'description' => $descr,
    ...
    public function postLimitTime()
  {
    ...
    $messages = [
    'timielimit.required' => trans('general.please_write_something'),
    'timielimit.max' => trans('general.please_write_something')
    ...
    $validator = Validator::make($input, [
      'timielimit'  => 'required|min:0|lte:duration'
    ...
    if ($validator->fails()) {
      ...
      $videoEmbed = $sql->media()->where('video', '<>', '')->first();
      $isVideoEmbed = false;

  		if ($videoEmbed) {
        // Update URL the Video
          $videoEmbed->timielimit = $input['timielimit'];
          $videoEmbed->save();
  		}
...
public function deletePostCat(Request $request){
    ...
    Updates::where('cat_post',$request->val)->whereUserId(auth()->id())->update(['cat_post' => Null]);
  	return response()->json([
                  'success' => true
                ]);
}
  ...
  public function image($id, $path)
    ...
    public function explore()
    {
      $updates = Updates::leftjoin('users', 'updates.user_id', '=', 'users.id')
          ...
          ->where('updates.expired', 'no') /*My changes*/
    ...
    public function stories(){
      ...
      $users = $this->userExplore();
      $title = trans('my_lang.promoted');

      return view('index.stories', ['updates' => $updates, 'users' => $users, 'title' => $title]);
    }
    /*My Changes*/
    public function storyExpired()
    {
      $id = $this->request->input('id');
      $expired = $this->request->input('expired');

      $up = Updates::whereId($id)->whereIsStory('yes')->update([
        'expired' => $expired
      ]);
    ...
    public function storePostViews()
    {
      $id = $this->request->input('id');
      ...
      if($id != ''){
      if(auth()->check()){
      $views = PostViews::firstOrNew(['user_id' => auth()->id(), 'media_id' => $id,'post_type' => $type]);
      if (!$views->exists) {
      ...
      $views = PostViews::firstOrNew(['user_id' => $session_id, 'media_id' => $id,'post_type' => $type]);
      if (!$views->exists) {
      ...
      $views->media_id = $id;
      $views->post_type = 'post';
      //$views->save();
      }

  		$user = Media::find($id);
    }
      $totalviews = number_format($user->views()->count());

      return response()->json([
				'success' => $save,
				'total' => $totalviews == 0 ? null : $totalviews
			]);
    }
