...
class AdminController extends Controller
{
	...
	public function edit($id)
	{
        ...
        $user_status = UserStatus::where('user_id', $id)->get()->toArray();
        $status = array();
        foreach($user_status as $sta){
            $status []= (array) $sta;
        }
    	...
    	return view('admin.edit-member', ['user' => $user, 'creator_status' => $creator_status, 'user_status' => $status]);
    ...
    $user->save();
    if($request->creator_status){
            UserStatus::where('user_id', $id)->delete();
            foreach($request->creator_status as $creator_status){
                
                $user_status = new UserStatus();
                $user_status->status_id = $creator_status;
	...
	public function salesRefund($id)
  {
	...
	public function creator_status(Request $request){
          $data = CreatorStatus::latest()->paginate(20);

		 return view('admin.creator_status', ['data' => $data]);
    }
    
    public function add_creator_status() 
	{
		return view('admin.add-creator-status');
	}//<--- END METHOD

	public function store_creator_status(Request $request) {

		$temp            = 'public/temp/'; // Temp
	  $path            = 'public/img-c-status/'; // Path General
		...
		$rules = array(
          'name'        => 'required',
	       // 'price'        => 'required',
	        'image'   => 'required|mimes:jpg,gif,png,jpe,jpeg,svg|dimensions:min_width=75,min_height=75',
        );
		...
		$extension       = $request->file('image')->getClientOriginalExtension();
	//	$type_mime_image = $request->file('image')->getMimeType();
	//	$sizeFile        = $request->file('image')->getSize();
		$thumbnail       = $request->slug.'-'.Str::random(32).'.'.$extension;

		if( $request->file('image')->move($temp, $thumbnail) ) {
		...
		$sql              = New CreatorStatus;
		$sql->title        = $request->name;
		$sql->status        = $request->status ?? 'off';
		$sql->icon       = $thumbnail;
		$sql->save();

		\Session::flash('success_message', __('Added Successfully!'));

    	return redirect('panel/admin/creator_status');

	}//<--- END METHOD
	
	public function edit_creator_status($id) {

		$creator_status = CreatorStatus::find($id);

		return view('admin.edit-creator-status')->with('creator_status', $creator_status);

	}//<--- END METHOD

	public function update_creator_status(Request $request)
	{
		$creator_status        = CreatorStatus::find($request->id);
		$temp            = 'public/temp/'; // Temp
	  $path            = 'public/img-c-status/'; // Path General

	  if(!isset($creator_status)) {
			return redirect('panel/admin/creator_status');
		}
		...
		$rules = array(
          'name'        => 'required',
	      //  'price'        => 'required',
	        'image'   => 'mimes:jpg,gif,png,jpe,jpeg,svg|dimensions:min_width=75,min_height=75',
	     );
		...
		//	$image = Image::make($temp.$thumbnail);
			...
			// Delete Old Image
			\File::delete($path.$creator_status->thumbnail);

			}// End File
		} // HasFile
		else {
			$thumbnail = $creator_status->icon;
		}

		// UPDATE CATEGORY
		$creator_status->title   = $request->name;
		$creator_status->status   = $request->mode ?? 'off';
		$creator_status->icon  = $thumbnail;
		$creator_status->save();
	...
	public function destroy_creator_status($id)
	{

			$creator_status   = CreatorStatus::findOrFail($id);
			$thumbnail    = 'public/img-c-status/'.$creator_status->icon; // Path General
