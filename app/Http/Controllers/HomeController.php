  ...
  public function __construct(Request $request, AdminSettings $settings)
  {
    ...
    {      
      ...
      if (auth()->guest()) {
          ...
          // Guests use the new public home on live; staging still uses the same shell.
          if ($isStagingApp || auth()->guest()) {
            $home = 'homenew';
          } else {
          ...
          $updates = Updates::leftjoin('users', 'updates.user_id', '=', 'users.id')
              ...
              ->where('updates.expired', 'no')
                ...
                })->when($this->request->input('query') == 'ajax-cat', function($q) {
                       
                       if($this->request->input('type') != 'all'){
                        $q->where('updates.cat_post', $this->request->input('type'));
                        $q->where('updates.user_id', auth()->user()->id);
                       }
              ...
              /*My Changes */
        		...
        		/*My changes*/
        		$updates = $updates->groupBy('updates.id')
         ...
         /*My Changes*/
          ...
          /*My Changes*/
            ...
            if($ajax == 'ajax-schedule' || $ajax == 'ajax-cat') {
                
                $counterPosts = ($updates->total() - $this->settings->number_posts_show);
                $total = $updates->total();
                $countUp = $updates->count(); 
                 return view('includes.updates', ['updates' => $updates,'countUp' => $countUp, 'users' => $users,  'ajaxRequest' => true, 'counterPosts' => $counterPosts, 'total' => $total])->render();
            } else{
    ...
    public function creators($type = false, $id = false)
    {
      if ($type === 'live') {
        return $this->creatorsBroadcastingLive();
      }
    ...
    case 'status':
            $orderBy = 'id';
            $creator_statu = CreatorStatus::find($id);
            $title = $creator_statu->title;
          break;
      ...
      if (strlen($query) >= 3) {
        ...
        $title = trans('general.search').' "'.$query.'"';
         ...
         $users = User::where('users.status','active')
              ->where('username','LIKE', '%'.$query.'%')
                ...
                ->where('id', '<>', $this->settings->hide_admin_profile == 'on' ? 1 : 0)
                ->whereRelation('plans', 'status', '1')
                ->where('free_subscription', 'no')
                ->where('hide_profile', 'no')
                ->where('blocked_countries', 'NOT LIKE', '%'.Helper::userCountry().'%')
                ->orWhere('users.status','active')
                  ->where('verified_id', 'yes')
                  ->where('id', '<>', $this->settings->hide_admin_profile == 'on' ? 1 : 0)
                  ->where('free_subscription', 'yes')
                  ->where('hide_profile', 'no')
        ...
        if ($type == 'free') {
          ...
          $data->where('users.status','active')
                 ->where('verified_id', 'yes')
                  ...
                  ->where('blocked_countries', 'NOT LIKE', '%'.Helper::userCountry().'%');
                  if ($type == 'status') {
                    // $user_status = UserStatus::where('status_id', $id)->get()->toArray();
                    $data->whereHas('user_status', function($data) use ($id) {
                    //$id= 4;
                      $data->where('user_status.status_id', $id);
                    });
                ...
                $data->orWhere('users.status','active')
                  ...
                  ->whereRaw($whereRawFeatured);
                  if ($type == 'status') {
                   // $data->leftjoin('user_status', 'user_status.user_id', '=', 'users.id');
                   // $user_status = $data->('user_status.status_id', $id)->get()->toArray();
                    $data->whereHas('user_status', function($data) use ($id) {
                    //$id= 4;
                      $data->where('user_status.status_id', $id);
                    });
          ...
          $users = $data->groupBy('users.id')
              ...
              ->select('users.cover','users.username','users.active_status_online','users.id','users.intro_video','users.avatar','users.name','users.hide_name','users.verified_id','users.featured',
              'users.hide_count_subscribers','users.hide_last_seen','users.last_seen','users.date','users.story','users.free_subscription','users.status','users.plan')
              ->paginate(10);
        }
      }
    	...
    	//My Changes added gender and $'type' => $type
    	$creator_status = CreatorStatus::orderBy('id')->get();
    	$genders = explode(',', $this->settings->genders);	
        return view('index.creators-new', [
          'users' => $users,
          'title' => $title,
          'type' => $type,
          'genders' => $genders,
    ...
    // My Changes 
    public function creatorsSearch($q = '', $type = false){
        
        $query = trim($this->request->input('q'));
        $type = trim($this->request->input('type'));
        $genders = explode(',', $query);
      ...
      case 'free':
        ...
        $online = 'no';
        if($query != ""){
            if(in_array('online', $genders)){
                  $online = 'yes';
                }
        //foreach($genders as $gender){
       
        $data = User::where('users.status','active');
            if((count($genders) === 1 && !in_array('online', $genders)) || (count($genders) > 1))
              $data->whereIn('gender',$genders);
                $data->whereVerifiedId('yes')
                ->where('id', '<>', $this->settings->hide_admin_profile == 'on' ? 1 : 0)
                ...
                ->orWhere('users.status','active')
                  ...
                  if((count($genders) === 1 && !in_array('online', $genders)) || (count($genders) > 1))
                  $data->whereIn('gender',$genders);
                  $data->whereHideName('no')
                  ->where('blocked_countries', 'NOT LIKE', '%'.Helper::userCountry().'%');
                $users = $data->orderBy('featured_date','desc')
                ->paginate(12);
  //  }
        } else{
            $data = User::where('users.status','active');
          ...
          $users =  $data->groupBy('users.id')
              ...
              ->paginate(12);
                ...
                return view('index.creators-search', [
          'users' => $users,
          'online' => $online
        ]);
    }
    ...
    public function category($slug, $type = false)
    {
     ...
     // My changes
      ...
      switch ($type) {
        ...
        $orderBy = 'id';
        $creator_statu = CreatorStatus::find($id);
        $title = $title.' - '.$creator_statu->title;
          break;
    ...
    public function categorySearch($slug, $type = false)
    {
        $query = trim($this->request->input('q'));
        $type = trim($this->request->input('type'));
        $slug = trim($this->request->input('cat'));
        $genders = explode(',', $query);
      $category = Categories::where('slug', '=', $slug)->where('mode','on')->firstOrFail();
      $title    = \Lang::has('categories.' . $category->slug) ? __('categories.' . $category->slug) : $category->name;
      $online = 'no';
      ...
      if ($type == 'free') {
        ...
        $data = User::where('users.status','active')
            ...
            $data->whereIn('gender',$genders);
              ...
              ->whereHideProfile('no')
              ->where('blocked_countries', 'NOT LIKE', '%'.Helper::userCountry().'%');
            $users = $data->orderBy($orderBy, 'desc')
            ->paginate(12);
          } else {
             $users = User::where('users.status','active')
            ->where('categories_id', 'LIKE', '%'.$category->id.'%')
          ...
          $data->where('users.status','active')
            ...
            ->orWhere('users.status','active')
              ->where('categories_id', 'LIKE', '%'.$category->id.'%');
                if($query != ""){
                    if((count($genders) === 1 && !in_array('online', $genders)) || (count($genders) > 1))
                     $data->whereIn('users.gender',$genders);
                }
              $data->whereVerifiedId('yes')
              ->where('users.id', '<>', $this->settings->hide_admin_profile == 'on' ? 1 : 0)
              ->whereFreeSubscription('yes')
              ...
              if (! $type) {
                ...
                $data->where('blocked_countries', 'NOT LIKE', '%'.Helper::userCountry().'%')
        ...
        if ($this->request->input('page') > $users->lastPage()) {
    		...
    		$genders = explode(',', $this->settings->genders);
        return view('index.creators-cat-search', [
          'users' => $users,
            'title' => $title,
            'slug' => $slug,
            'image' => $category->image,
    ...
    public function searchCreator()
    {
      ...
      //My Changes 
      public function categorySearchLive($type = false)
      {
        // Search Live Streaming
        $query = trim($this->request->input('q'));
        $type = trim($this->request->input('type'));
      ...
      if(in_array('online', $genders)){
        ...
        $data = LiveStreamings::where('live_streamings.updated_at', '>', now()->subMinutes(5))
        ->leftjoin('users', 'users.id', '=', 'live_streamings.user_id')
        ->where('live_streamings.status', '0');
        if((count($genders) === 1 && !in_array('online', $genders)) || (count($genders) > 1))
            $data->whereIn('users.gender',$genders);
        $data->where('users.blocked_countries', 'NOT LIKE', '%'.Helper::userCountry().'%');
        $users = $data->orderBy('live_streamings.id', 'desc')
        ->select('live_streamings.*')
        ->paginate(20);
        ...
        return view('index.creators-search-live', [
          'users' => $users,
         // 'genders' => $genders,
          'online' => $online
        ]);
