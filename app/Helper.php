...
public static function resizeImageFixed($image,$width,$height,$imageNew = null)
{
    ...
    /*My Changes*/
	public static function removeTagDiv($html)
	{
			  //parsing begins here:
				$doc = new \DOMDocument();
				@$doc->loadHTML(mb_convert_encoding($html, 'HTML-ENTITIES', 'UTF-8'));
                ...
                foreach($xpath->evaluate("//div") as $node) {
	...
	{
		$settings = AdminSettings::select('currency_position', 'currency_symbol')->first();
 ...
 public static function daysInMonth($month, $year)
 {
	...
	{
		$settings = AdminSettings::select('wallet_format')->first();
	...
	public static function equivalentMoney($walletFormat)
	{
		...
		switch ($walletFormat) {
			case 'credits':
				return __('general.credit_equivalent_money') . ' ' .self::amountFormatDecimal(1) .' '.$settings->currency_code;
				break;

			case 'points':
				return __('general.point_equivalent_money') . ' ' .self::amountFormatDecimal(1) .' '.$settings->currency_code;
				break;

			case 'tokens':
				return __('general.token_equivalent_money') . ' ' .self::amountFormatDecimal(1) .' '.$settings->currency_code;
				break;
	...
	{
		$settings = AdminSettings::select('wallet_format', 'currency_code')->first();
	...
	{
		$settings = AdminSettings::select('wallet_format', 'currency_position', 'currency_symbol')->first();

		if ($settings->wallet_format == 'real_money') {
			return $settings->currency_position == 'left'  ? $settings->currency_symbol : (($settings->currency_position == 'left_space') ? $settings->currency_symbol.' ' : null);
		}
	...
	/*My Changes*/
	public static function checkTextEmbed($str, $url = null)
	{
		$output = preg_replace('/<script.+?<\/script>/im', "", $str);
        $output = preg_replace('/<embed.+?<\/embed>/im', "", $output);
        $output = preg_replace('/<object.+?<\/object>/im', "", $output);
        $output = preg_replace('/<blockquote.+?<\/blockquote>/im', "", $output);
        $output = preg_replace("/<div>(.*?)<\/div>/", "$1", $output);

         
		return $output;
	}
	public static function checkTextEmbedYou($str, $url = null)
	{
		$output = preg_replace('/<script.+?<\/script>/im', "", $str);
        $output = preg_replace('/<embed.+?<\/embed>/im', "", $output);
        $output = preg_replace('/<object.+?<\/object>/im', "", $output);
        $output = preg_replace('/<iframe.+?<\/iframe>/im', "", $output);
	...
	public static function cleanTextEmbed($string, $url = null)
	{

		 $tmp = preg_match_all('/(<blockquote \w*)(.*)(<\/script>)/', $string, $matches); 
         
		return isset($matches[0][0]) ? $matches[0][0] : false;
	}
	/*My Changes*/
	public static function gettiktokId($url)
	{
		$url = explode('/',$url);
			 $slashCount =  count($url) -1; // added by AMR
			 $trail = ''; // added by AMR until dev finds solution
    ...
    public static function txt2link($string){
