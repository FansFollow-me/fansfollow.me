...
@section('css')
  <script type="text/javascript">
      /*My Changes*/
      var isMessageChat = true;
      var subscribed_active = {{ $subscribedToYourContent || $subscribedToMyContent || auth()->user()->isSuperAdmin() || $user->isSuperAdmin() || auth()->user()->verified_id == 'yes'? 'true' : 'false' }};
...
<section class="section section-sm pb-0 h-100 section-msg position-fixed">
    <div class="container h-100">
      ...
      <div class="row mb-3">
        <div class="col-12">
          <div class="profile-meta d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
            <div>
              <p class="text-uppercase text-muted mb-2 small font-weight-bold">{{ trans('general.messages') }}</p>
              <h2 class="mb-1">{{$user->hide_name == 'yes' ? $user->username : $user->name}}</h2>
              <p class="mb-0 text-muted">{{ trans('general.messages_subtitle') }}</p>
            </div>
            <a href="{{url($user->username)}}" class="btn btn-outline-primary">{{ trans('general.profile') }}</a>
          </div>
        </div>
      </div>
      @endif
  ...
  <div class="card w-100 rounded-0 h-100 border-top-0">
    <div class="card-header bg-white pt-4">
      <div class="media">
        ...
        @if ($messages->count() != 0)
					...
					{!! Form::open([
						'method' => 'POST',
						'url' => "conversation/block/$user->id",
						'class' => 'd-inline'
					]) !!}
					
                    @if(!auth()->user()->isBlocked($user->id))
					{!! Form::button('<i class="fa fa-ban mr-2"></i> '.trans('my_lang.block'), ['class' => 'dropdown-item actionBlock']) !!}
					@else
					{!! Form::button('<i class="fa fa-ban mr-2"></i> '.trans('my_lang.un_block'), ['class' => 'dropdown-item actionUnBlock']) !!}
					@endif
					{!! Form::close() !!}
    ...
    @endif
      ...
      @if (! auth()->user()->checkRestriction($user->id))
          ...
          <!--My Changes -->
          @if(! auth()->user()->checkBlock($user->id))
          @if ($subscribedToYourContent || $subscribedToMyContent || auth()->user()->isSuperAdmin() || $user->isSuperAdmin() || auth()->user()->verified_id == 'yes' || $user->non_subscribers_can_message_tip == 'yes')
            ...
            <!--My Changes -->
            @if(!auth()->user()->isSuperAdmin() && $user->non_subscribers_can_message_tip == 'yes')
            ...
            <!-- Alert -->
            <div class="row">
            <div id="collapseExample" class="alert alert-info col-md-10" id="" style="padding: 0.25rem; top: -13px;">
             {{$user->name}} requests {{ Helper::amountFormatDecimal($user->tip_amount) }} Per Message for Non-Subscribers,
							{{ trans('general.wallet') }}: <span class="balanceWallet">{{Helper::userWallet()}}</span> <i class="bi-info-circle"></i>
           </div><!-- Alert -->
           <span class="col-md-2" style="left: -15px; top: -18px;" data-toggle="collapse" href="#collapseExample">
           ...
           @endif
            ...
            @if (auth()->check() && auth()->id() != $user->id
                ...
                <input type="hidden" name="is_tip" id="is_tip" value="yes">
                <input type="hidden" name="amount" id="amount" value="{{$user->tip_amount}}">
          ...
          <div class="d-inline-block float-right rounded-pill mt-3 position-relative">
            ...
            <span class="badge badge-danger mt-2 mr-1" style="font-size: 14px">{{ Helper::amountFormatDecimal($user->tip_amount) }}  </span>
            <button type="submit" id="button-reply-msg" disabled data-tip="yes" data-send="{{ trans('auth.send') }}" data-wait="{{ trans('general.send_wait') }}" class="btn btn-sm btn-primary rounded-pill float-right e-none">
      ...
      @else
        ...
        <div class="alert alert-primary m-0 alert-dismissible fade show" role="alert">
          <i class="fa fa-info-circle mr-2"></i>
          @if($user->blocked_by == auth()->user()->id)
        {!! trans('my_lang.show_form_msg_error_block_me', ['user' => '<a href="'.url($user->username).'" class="link-border text-white">'.$user->first_name.'</a>']) !!}
        @else
        {!! trans('my_lang.show_form_msg_error_block_', ['user' => '<a href="'.url($user->username).'" class="link-border text-white">'.$user->first_name.'</a>']) !!}
        @endif
      </div>
        @endif
        <!--My Changes end-->
