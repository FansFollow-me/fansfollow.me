    ...
    <div class="container">
      ...
      <div class="mb-4">
        <div class="d-flex flex-column flex-lg-row align-items-lg-end justify-content-between gap-3">
          <div>
            <p class="text-uppercase text-muted mb-2 small font-weight-bold">{{ trans('admin.dashboard') }}</p>
            <h2 class="mb-1">{{ auth()->user()->hide_name == 'yes' ? auth()->user()->username : auth()->user()->name }}</h2>
            <p class="text-muted mb-0">{{ trans('general.dashboard_desc') }}</p>
          </div>
          <div class="d-flex gap-2">
            <a href="{{ url(auth()->user()->username) }}" class="btn btn-outline-primary">{{ trans('general.my_page') }}</a>
            <a href="{{ url('settings') }}" class="btn btn-primary">{{ trans('users.settings') }}</a>
          </div>
        </div>
      </div>
      @endif
