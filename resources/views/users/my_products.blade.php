...
<div class="mb-4">
  <div class="d-flex flex-column flex-lg-row align-items-lg-end justify-content-between gap-3">
    <div>
      <p class="text-uppercase text-muted mb-2 small font-weight-bold">{{ trans('general.products') }}</p>
      <h2 class="mb-1">{{ trans('general.all_products_published') }}</h2>
      <p class="text-muted mb-0">Manage live offers, new product launches, and fulfillment status from one place.</p>
    </div>
    <div class="d-flex gap-2">
      <a href="{{ url('my/sales') }}" class="btn btn-outline-primary">{{ trans('general.sales') }}</a>
      <a href="{{ url('add/product') }}" class="btn btn-primary">{{ trans('general.add_product') }}</a>
    </div>
  </div>
</div>
@endif
              ...
              <tbody>
                ...
                @foreach ($products as $product)
                  ...
                  <tr>
                    ...
                    <td>{{ ($product->type == 'digital') ? __('general.digital_download') 
                    : (($product->type == 'physical') ? __('general.physical_products')
                    : (($product->type == 'video') ? __('my_lang.video_calls')
                    : (($product->type == 'snapchat') ? __('my_lang.snapchat')
                    : (($product->type == 'garments') ? __('my_lang.garments')
                    : __('general.custom_content'))))) }}</td>
