...
<div class="jumbotron jumbotron-cover-user home m-0 position-relative"
          style="padding: 240px 0; background: #ececec @if ($user->cover != '') url({{ Helper::getFile(config('path.cover').$user->cover) }}) no-repeat center center; background-size: cover; @endif">
      ...
      <div class="col-12 col-xl-10">
        <div class="profile-meta mb-4">
          <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
            <div>
              <p class="text-uppercase text-muted small font-weight-bold mb-2">{{ trans('general.profile') }}</p>
              <h2 class="mb-1">{{ $user->hide_name == 'yes' ? $user->username : $user->name }}</h2>
              <p class="mb-0 text-muted">{{ strip_tags($user->story) }}</p>
            </div>
            <div class="d-flex flex-wrap gap-2">
              @if (auth()->check() && auth()->id() == $user->id)
                <a class="btn btn-primary" href="{{ url('settings') }}">{{ trans('users.edit_profile') }}</a>
                <a class="btn btn-outline-primary" href="{{ url('my/posts') }}">{{ trans('general.posts') }}</a>
              @else
                <a class="btn btn-primary" href="{{ url('messages') }}">{{ trans('general.messages') }}</a>
              @endif
            </div>
          </div>
        </div>
        ...
        <div class="w-100 text-center py-4 img-profile-user">
          <div class="avatar-outer-wrap" >
            <div class="avatar-inner-wrap" >
              <div @if ($user->isLive() && auth()->check() && auth()->id() != $user->id) data-url="{{ url('live', $user->username) }}" @endif class="text-center position-relative @if ($user->isLive() && auth()->check() && auth()->id() != $user->id) avatar-wrap-live liveLink @else avatar-wrap @endif shadow @if (auth()->check() && auth()->id() != $user->id && Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes' || auth()->guest() && Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes') user-online-profile overflow-visible @elseif (auth()->check() && auth()->id() != $user->id && !Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes' || auth()->guest() && !Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes') user-offline-profile overflow-visible @endif">
                ...
                @if (auth()->check()  && $user->intro_video != NULL && auth()->id() != $user->id)
                  <a href="javascript:void(0);" data-toggle="modal" data-target="#introVidModal{{$user->username}}" class="w-100">
                      <span class="position-absolute intro-video-span-creator"><img src="{{Helper::getFile('img/play-button.png')}}" width="30" height="30"></span>
                  </a>
                  @php $response = $user; @endphp
                ...
                <div class="progress-upload">0%</div>
                  @if (auth()->check() && auth()->user()->status == 'active' && auth()->id() == $user->id)
                    ...
                    @if (auth()->check()  && $user->intro_video != NULL)
                      <a href="javascript:void(0);" data-toggle="modal" data-target="#introVidModal{{$user->username}}" class="video-modal-btn">
                        <span class="intro-video-span-creator"><img src="{{Helper::getFile('img/play-button.png')}}" width="30" height="30"></span>
                      </a>
                      @php $response = $user; @endphp
                      @include('includes.modal-intro-video')
                    @endif
              ...
              <h4>
                ...
                @if ($user_status)
                                @foreach($user_status as $status)
                                    <small class="text-featured" title="{{ ucfirst($status->title) }}"
                                        data-toggle="tooltip" data-placement="top">
                                        <img class="image-responsive" src="{{ Helper::getFile('img-c-status/'. $status->icon) }}" alt="{{$status->title}}" width="30px"/>
                                    </small>
                                @endforeach
                                @endif
              </h4>
          ...
          <div class="about-wrap"> 
            ...
            @if (auth()->check() && auth()->id() == $user->id)
              ...
              <a href="{{url('settings')}}" class="btn btn-primary btn-profile mr-1"><i class="fas fa-pencil-alt mr-2"></i> {{ auth()->user()->verified_id == 'yes' ? trans('general.edit_my_page') : trans('users.edit_profile')}}</a>
            ...
            @endif
              ...
              @if ($userPlanMonthlyActive
               ...
               @if (auth()->check() && auth()->id() != $user->id
                  ...
                  && $user->updates()->where('updates.expired', 'no')
                ...
                <a href="javascript:void(0);" data-toggle="modal" data-target="#subscriptionForm" class="btn btn-primary btn-profile mr-1">
                  ...
                  <i class="feather icon-unlock mr-1"></i> {{trans('general.subscribe_month', ['price' => Helper::formatPrice($user->plan('monthly', 'price'))])}}
              ...
              @elseif (auth()->check() && auth()->id() != $user->id && ! $checkSubscription && $paymentIncomplete)
                ...
                <a href="{{ route('cashier.payment', $paymentIncomplete->last_payment) }}" class="btn btn-warning btn-profile mr-1">
                  ...
                  <i class="fa fa-exclamation-triangle mr-1"></i> {{trans('general.confirm_payment')}}
              ...
              @elseif (auth()->check() && auth()->id() != $user->id && $user->free_subscription == 'yes' && $user->updates()->where('updates.expired', 'no')
              ...
              @elseif (auth()->guest() && $user->updates()->where('updates.expired', 'no')
            ...
            @if (auth()->check() && auth()->id() != $user->id && $user->updates()->where('updates.expired', 'no')
                ...
                })->count() <> 0 && $settings->disable_tips == 'off') <!--My Changes -->

              <a href="javascript:void(0);" data-toggle="modal" title="{{trans('general.tip')}}" data-target="#tipForm" class="btn btn-google btn-tip btn-profile mr-1" data-cover="{{Helper::getFile(config('path.cover').$user->cover)}}" data-avatar="{{Helper::getFile(config('path.avatar').$user->avatar)}}" data-name="{{$user->hide_name == 'yes' ? $user->username : $user->name}}" data-userid="{{$user->id}}">

                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-coin mr-1" viewBox="0 0 16 16">
            ...
            @elseif (auth()->guest() && $user->updates()->where('updates.expired', 'no')
              ...
              <a href="{{url('login')}}" data-toggle="modal" data-target="#loginFormModal" class="btn btn-google btn-tip btn-profile mr-1" title="{{trans('general.tip')}}">
            ...
            @if (auth()->guest() && $user->verified_id == 'yes' || auth()->check() && auth()->id() != $user->id && $user->verified_id == 'yes')
              ...
              <button @guest data-toggle="modal" data-target="#loginFormModal" @else id="sendMessageUser" @endguest data-url="{{url('messages/'.$user->id, $user->username)}}" title="{{trans('general.message')}}" class="btn btn-google btn-message btn-profile mr-1">

                <i class="fa fa-envelope mr-1"></i> <span>{{trans('general.message')}}</span>
            ...
            @if ($user->verified_id == 'yes')
              ...
              <button class="btn btn-profile btn-google" title="{{trans('general.share')}}" id="dropdownUserShare" role="button" data-toggle="modal" data-target=".share-modal">
                ...
                <i class="fa fa-share-alt mr-1"></i> <span>{{trans('general.share')}}</span>
          ...
          <ul class="info-profile modal-btns mb-2">
                @if ($likeCount != 0 || $subscriptionsActive != 0)
                  <!--My changes -->
                  @if ($likeCount != 0)
                    <li><img src="{{url('public/img/heart.png')}}" width="25px" role="presentation" /> {{ Helper::formatNumber($likeCount) }} {{ __('general.likes') }}</li>
                  @endif
                  @if ($subscriptionsActive != 0 && $user->hide_count_subscribers == 'no')
                    <li><img src="{{url('public/img/subscribe.png')}}" width="25px" role="presentation" /> {{ Helper::formatNumber($subscriptionsActive) }} {{ trans_choice('general.subscribers', $subscriptionsActive) }}</li>
                  @endif
                  <!--My Changes End-->
                @endif

                @if ($user->show_views == 'yes' || (isset($user->country()->country_name) && $user->hide_my_country == 'no'))
                  @if ($user->show_views == 'yes')
                    <li>
                      <img src="{{url('public/img/star.png')}}" width="25px" role="presentation"  alt="Face Icon"/>{{ __('my_lang.views_profile') }}: {{Helper::formatNumber($user->views)}}
                    </li>
                  @endif
                  <!--My Changes End-->
                  @if (isset($user->country()->country_name) && $user->hide_my_country == 'no')
                    <!--My changes -->
                    <li>
                      <img src="{{url('public/img/pin.png')}}" width="25px"  role="presentation" alt="Country Icon"/> {{$user->country()->country_name}}
                    </li>
                  @endif
                @endif
                ...
                @if ($user->profession != '' && $user->verified_id == 'yes')
                  <li>
                    <img src="{{url('public/img/briefcase.png')}}" width="25px"  role="presentation" alt="Profession"/>  {{$user->profession}}
                  </li>
                @endif
                @if ($user->show_my_birthdate == 'yes')
                  <!--My changes style="font-size: 110%; font-weight: 500;" -->
                  <li class="btn-block justify-content-center">
                    <i class="far fa-calendar-alt"></i> {{ trans('general.birthdate') }} {{ Helper::formatDate($user->birthdate) }} ({{ \Carbon\Carbon::parse($user->birthdate)->age }} {{ __('general.years') }})
                  </li>
                @endif
              </ul>
        </div><!-- media-body -->
                                ...
                                <div class="card-body">
                                    <div class="my-socials">
                                        ...
                                        <div class="card-text row">
                                            @if ($user->facebook != '')
                                                <div class="col-2">
                                                    <a href="https://facebook.com/{{ $user->facebook }}"
                                                        title="https://facebook.com/{{ $user->facebook }}"
                                                        target="_blank" class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/FACEBOOK.svg') }}"
                                                            class="mr-1" alt="Facebook" /></a>
                                                </div>
                                            @endif
                                            ...
                                            @if ($user->twitter != '')
                                                <div class="col-2">
                                                    <a href="https://twitter.com/{{ $user->twitter }}"
                                                        title="https://twitter.com/{{ $user->twitter }}" target="_blank"
                                                        class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/TWITTER.svg') }}"
                                                            class="mr-1" alt="Twitter" /></a>
                                                </div>
                                            @endif
                                            ...
                                            @if ($user->instagram != '')
                                                <div class="col-2">
                                                    <a href="https://instagram.com/{{ $user->instagram }}"
                                                        title="https://instagram.com/{{ $user->instagram }}"
                                                        target="_blank" class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/INSTAGRAM.svg') }}"
                                                            class="mr-1" alt="INSTAGRAM" /></a>
                                                </div>
                                            @endif
                                            ...
                                            @if ($user->youtube != '')
                                                <div class="col-2">
                                                    <a href="https://youtube.com/{{ $user->youtube }}"
                                                        title="https://youtube.com/{{ $user->youtube }}" target="_blank"
                                                        class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/YOUTUBE.svg') }}"
                                                            class="mr-1" alt="YOUTUBE" /></a>
                                                </div>
                                            @endif

                                            @if ($user->pinterest != '')
                                                <div class="col-2">
                                                    <a href="https://pinterest.com/{{ $user->pinterest }}"
                                                        title="https://pinterest.com/{{ $user->pinterest }}"
                                                        target="_blank" class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/PINTEREST.svg') }}"
                                                            class="mr-1" alt="PINTEREST" /></a>
                                                </div>
                                            @endif

                                            @if ($user->github != '')
                                                <div class="col-2">
                                                    <a href="https://onlyfans.com/{{ $user->github }}"
                                                        title="https://onlyfans.com/{{ $user->github }}" target="_blank"
                                                        class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/ONLYFANS.svg') }}"
                                                            class="mr-1" alt="onlyfans" /></a>
                                                </div>
                                            @endif
                                            ...
                                            @if ($user->snapchat != '')
                                                <div class="col-2">
                                                    <a href="https://www.snapchat.com/add/{{ $user->snapchat }}"
                                                        title="https://www.snapchat.com/add/{{ $user->snapchat }}"
                                                        target="_blank" class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/SNAPCHAT.svg') }}"
                                                            class="mr-1" alt="SNAPCHAT"  /></a>
                                                </div>
                                            @endif
                                            ...
                                            @if ($user->tiktok != '')
                                                <div class="col-2">
                                                    <a href="https://www.tiktok.com/{{ '@' . $user->tiktok }}"
                                                        title="https://www.tiktok.com/{{ '@' . $user->tiktok }}"
                                                        target="_blank" class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/TIKTOK.svg') }}"
                                                            class="mr-1" alt="TIKTOK"  /></a>
                                                </div>
                                            @endif
                                            ...
                                            @if ($user->telegram != '')
                                                <div class="col-2">
                                                    <a href="https://t.me/{{ $user->telegram }}"
                                                        title="https://t.me/{{ $user->telegram }}" target="_blank"
                                                        class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/TELEGRAM.svg') }}"
                                                            class="mr-1" alt="TELEGRAM"  /></a>
                                                </div>
                                            @endif

                                            @if ($user->twitch != '')
                                                <div class="col-2">
                                                    <a href="https://www.twitch.tv/{{ $user->twitch }}"
                                                        title="https://www.twitch.tv/{{ $user->twitch }}"
                                                        target="_blank" class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/TWITCH.svg') }}"
                                                            class="mr-1" alt="TWITCH"  /></a>
                                                </div>
                                            @endif

                                            @if ($user->discord != '')
                                                <div class="col-2">
                                                    <a href="https://discord.gg/{{ $user->discord }}"
                                                        title="https://discord.gg/{{ $user->discord }}" target="_blank"
                                                        class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/DISCORD.svg') }}"
                                                            class="mr-1" alt="DISCORD"  /></a>
                                                </div>
                                            @endif
                                            ...
                                            @if ($user->vk != '')
                                                <div class="col-2">
                                                    <a href="https://vk.com/{{ $user->vk }}"
                                                        title="https://vk.com/{{ $user->vk }}" target="_blank"
                                                        class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/VK.com.svg') }}"
                                                            class="mr-1" alt="VK"  /></a>
                                                </div>
                                            @endif
                                            ...
                                            @if ($user->reddit != '')
                                                <div class="col-2">
                                                    <a href="https://reddit.com/user/{{ $user->reddit }}"
                                                        title="https://reddit.com/user/{{ $user->reddit }}"
                                                        target="_blank" class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/REDDIT.svg') }}"
                                                            class="mr-1" alt="REDDIT"  /></a>
                                                </div>
                                            @endif

                                            @if ($user->spotify != '')
                                                <div class="col-2">
                                                    <a href="https://spotify.com/{{ $user->spotify }}"
                                                        title="https://spotify.com/{{ $user->spotify }}" target="_blank"
                                                        class="text-muted share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/SPOTIFY.svg') }}"
                                                            class="mr-2" alt="SPOTIFY"  /></a>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div class="card-text">
                                            @if ($user->website != '')
                                                <div class="d-block mb-1 text-truncate">
                                                    <a href="http://{{ Helper::removeHTPP($user->website) }}"
                                                        title="{{ $user->website }}" target="_blank"
                                                        class="text-dark share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/LINKS.svg') }}"
                                                            class="mr-1" alt="LINKS"  />
                                            ...
                                            @if ($user->website2 != '')
                                                <div class="d-block mb-1 text-truncate">
                                                    <a href="http://{{ Helper::removeHTPP($user->website2) }}"
                                                        title="{{ $user->website2 }}" target="_blank"
                                                        class="text-dark share-btn-user"><img
                                                            src="{{ url('public/SOCIAL_ICONS/LINKS.svg') }}"
                                                            class="mr-1" alt="LINKS"  />
                                                        {{ Helper::removeHTPP($user->website2) }}</a>
                                                </div>
                                            @endif

                                            @if ($user->website3 != '')
                                                <div class="d-block mb-1 text-truncate">
                                                    <a href="http://{{ Helper::removeHTPP($user->website3) }}"
                                                        title="{{ $user->website3 }}" target="_blank"
                                                        class="text-dark share-btn-user"> <img src="{{ url('public/SOCIAL_ICONS/WISHLIST.svg') }}"
                                                            class="mr-1" alt="WISHLIST"  />
                                                        {{ Helper::removeHTPP($user->website3) }}</a>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
...
<ul class="nav nav-profile- nav-profile-tab justify-content-center nav-fill">
  ...
  <li class="nav-link @if (request()->path() == $user->username)active @endif navbar-user-mobile">
    ...
    <small class="btn-block sm-btn-size">{{Helper::formatNumber($user->updates()->where('updates.expired', 'no')
      ...
      <a class="tab-link" href="{{request()->path() == $user->username ? 'javascript:;' : url($user->username)}}" title="{{trans('general.posts')}}"><i class="feather icon-file-text"></i> <span class="d-lg-inline-block d-none">{{trans('general.posts')}}</span></a>
    ...
    <li class="nav-link @if (request()->path() == $user->username.'/photos')active @endif navbar-user-mobile">
      ...
      <a class="tab-link" href="{{request()->path() == $user->username.'/photos' ? 'javascript:;' : url($user->username, 'photos')}}" title="{{trans('general.photos')}}"><i class="feather icon-image"></i> <span class="d-lg-inline-block d-none">{{trans('general.photos')}}</span></a>
    ...
    <li class="nav-link @if (request()->path() == $user->username.'/videos')active @endif navbar-user-mobile">
      ...
      <a class="tab-link" href="{{request()->path() == $user->username.'/videos' ? 'javascript:;' : url($user->username, 'videos')}}" title="{{trans('general.video')}}"><i class="feather icon-video"></i> <span class="d-lg-inline-block d-none">{{trans('general.videos')}}</span></a>
    ...
    <li class="nav-link @if (request()->path() == $user->username.'/audio')active @endif navbar-user-mobile">
      ...
      <a class="tab-link" href="{{request()->path() == $user->username.'/audio' ? 'javascript:;' : url($user->username, 'audio')}}" title="{{trans('general.audio')}}"><i class="feather icon-mic"></i> <span class="d-lg-inline-block d-none">{{trans('general.audio')}}</span></a>
    ...
    @if ($settings->shop || ! $settings->shop && $userProducts->count() != 0)
         ...
         <li class="nav-link @if (request()->path() == $user->username.'/shop')active @endif navbar-user-mobile">
          ...
          <a class="tab-link" data-type="shop" href="{{request()->path() == $user->username.'/shop' ? 'javascript:;' : url($user->username, 'shop')}}" title="{{trans('general.shop')}}"><i class="feather icon-shopping-bag"></i> <span class="d-lg-inline-block d-none">{{trans('general.shop')}}</span></a>
...
@endif
        ...
        <!--My Changes && request()->path() == $user->username-->
        ...
        <div class="grid-updates position-relative" id="updatesPaginator">
            ...
            @php $countUp = $updates->count(); @endphp
        ...
        @endif   
         ...
         <!--my changes closed div schedule-post-div-->
              ...
              <div class="card-body px-lg-5 py-lg-5 position-relative">
                ...
                <div class="text-muted text-center mb-3 position-relative modal-offset">
                  ...
                  <h6 class="font-weight-light">
                    ...
                    {!! trans('general.subscribe_month', ['price' => '<span class="font-weight-bold">'.Helper::formatPrice($user->plan('monthly', 'price'), true).'</span>']) !!} {{trans('general.unlocked_content')}} {{$user->hide_name == 'yes' ? $user->username : $user->name}}
                ...
                <!--My Changes formSubscription_-->

                <form method="post" action="{{url('buy/subscription')}}" id="formSubscription_">
                  ...
                  <input name="interval" value="monthly" id="plan-monthly_1" class="d-none" type="radio">
                ...
                @endif
                  ...
                  <div class="alert alert-danger display-none" id="error1">

                      <ul class="list-unstyled m-0" id="showErrors1"></ul>
                ...
                @endif
                 ...
                 <!--My Changes-->
                    ...
                    <button type="submit" class="btn btn-primary mt-4 w-100 subscriptionBtn1" onclick="$('#plan-monthly_1').trigger('click');">
                    ...
                    @if ($plans->count())
                      ...
                      <div class="collapse" id="collapseSubscriptionBundles">
                        ...
                        @foreach ($plans as $plan)
                          ...
                          <button type="submit" class="btn btn-primary mt-2 w-100 subscriptionBtn1" onclick="$('#plan-{{$plan->interval}}').trigger('click');">
