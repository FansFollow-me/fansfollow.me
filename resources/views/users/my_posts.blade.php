...
<section class="section section-sm">
    <div class="container">
      @if ($isStagingDesign)
      <div class="mb-4">
        <p class="text-uppercase text-muted mb-2 small font-weight-bold">{{ trans('general.my_posts') }}</p>
        <h2 class="mb-1">{{ trans('general.all_post_created') }}</h2>
        <p class="text-muted mb-0">Review published, scheduled, and locked posts with their engagement stats.</p>
      </div>
      @endif
          ...
          <div class="btn-block mb-3 text-right">
                <span>
                  ...
                  <select class="ml-2 custom-select w-auto" id="filter">
                      <option @if (! request()->get('sort')) selected @endif value="{{url('my/posts')}}">{{trans('general.all')}}</option>
                        <option @if (request()->get('sort') == 'active') selected @endif value="{{url('my/posts')}}?sort=active">{{trans('general.active')}}</option>
                      <option @if (request()->get('sort') == 'schedule') selected @endif value="{{url('my/posts')}}?sort=schedule">Schedule</option>
                    </select>
                </span>
          </div>
          <!--My Changes end-->
                ...
                @foreach ($posts as $post)
                  ...
                  <tr>
                    ...
                    <a href="{{ url($post->user()->username, 'post').'/'.$post->id }}" target="_blank">
                      {!! str_limit($post->description, 20, '...') !!} <i class="bi bi-box-arrow-up-right ml-1"></i><!--My Changes-->
                    </a>
                    </td>
                    <td>
                      @if ($post->locked == 'yes')
                    ...
                    <td>
                      @if ($post->is_schedule == 'yes' && $post->schedule_date_time > date('Y-m-d H:i:00'))
                      <span class="badge badge-pill badge-danger text-uppercase">{{trans('general.pending')}}</span>
