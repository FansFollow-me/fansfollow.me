...
<footer class="container">
  ...
  @endif
    <div class="col-md-3">
      ...
      @guest
      <!--My changes-->
        <li><a class="link-footer" data-toggle="modal" data-target="#loginFormModal" href="{{$settings->home_style == 0 ? url('login') : url('/')}}">{{ trans('auth.login') }}</a></li><li>
          @if ($settings->registration_active == '1')
          <!--My changes-->
        <li><a class="toggleRegister link-footer" data-toggle="modal" data-target="#loginFormModal" href="{{$settings->home_style == 0 ? url('signup') : url('/')}}">{{ trans('auth.sign_up') }}</a></li><li>
        @endif
        @else
          ...
          <li><a class="link-footer" href="{{ url('settings') }}">{{ auth()->user()->verified_id == 'yes' ? trans('general.edit_my_page') : trans('users.edit_profile')}}</a></li><li>
