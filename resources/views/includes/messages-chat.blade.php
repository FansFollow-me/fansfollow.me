...
@foreach ($messages as $msg)
 ...
 //My Changes txt2link
  $chatMessage = $msg->message ? Helper::txt2link(Helper::checkText($msg->message)) : null;
...
<div class="media-body position-relative">
  ...
  @endif
  <!--My changes -->
  <div style="font-size: 1.1rem" class="@if ($mediaCount == 0) float-right @else wrapper-msg-left @endif message position-relative text-word-break @if ($mediaCount == 0 && $msg->tip == 'no') bg-primary @else media-container @endif text-white @if ($msg->format == 'zip') w-50 @else w-auto @endif  rounded-bottom-right-0">
  ...
  @if ($mediaCount != 0 && $msg->message != '')
    <div class="w-100 d-inline-block">
      <!--My Chnages-->
      <div style="font-size: 1.1rem" class="w-auto position-relative text-word-break message bg-primary float-right text-white rounded-top-right-0">
        {!! $chatMessage !!}
      </div>
    </div>
@endif
    ...
    <span class="w-100 d-block text-muted float-right text-right pr-1 small">
      ...
      <!--My Changes-->
      @if ($msg->price != 0.00 && $msg->tip_chat == 'yes')
    {{ Helper::amountFormatDecimal($msg->price) }} {{ $checkPayPerView ? __('general.paid') : null }} <i class="feather icon-lock mr-1"></i> -
    {{ Helper::amountFormatDecimal($msg->tip_amount) }} {{__('general.tip')}} -
    @elseif($msg->price != 0.00 && $msg->tip_chat != 'yes')
    {{ Helper::amountFormatDecimal($msg->price) }} {{ $checkPayPerView ? __('general.paid') : null }} <i class="feather icon-lock mr-1"></i> -
    @elseif($msg->price == 0.00 && $msg->tip_chat == 'yes')
       {{ Helper::amountFormatDecimal($msg->tip_amount) }} {{__('general.tip')}} -
  @endif
  ...
  @if ($msg->price != 0.00 && ! $checkPayPerView)
    ...
    <div class="btn-block p-sm text-center content-locked mb-2 pt-lg pb-lg px-3 {{$textWhite}} custom-rounded float-left" style="{{$backgroundPostLocked}}">
        ...
        <a href="javascript:void(0);" data-toggle="modal" data-target="#payPerViewForm" data-mediaid="{{$msg->id}}" data-price="{{Helper::formatPrice($msg->price, true)}}" data-subtotalprice="{{Helper::formatPrice($msg->price)}}" data-pricegross="{{$msg->price}}" class="btn btn-primary w-100">
          <i class="feather icon-unlock mr-1"></i> {{ trans('general.unlock_for') }} {{Helper::formatPrice($msg->price)}}
        </a>
...
@if ($msg->price == 0.00 || $msg->price != 0.00 && $checkPayPerView)
   <!--My changes style="font-size: 1.1rem"-->
  <div style="font-size: 1.1rem" class="@if ($mediaCount == 0) float-left @else wrapper-msg-right @endif message position-relative text-word-break @if ($mediaCount == 0 && $msg->tip == 'no') bg-light @else media-container @endif @if ($msg->format == 'zip') w-50 @else w-auto @endif rounded-bottom-left-0">
        @include('includes.media-messages')
  </div>
  @endif
  ...
  @if ($mediaCount != 0 && $msg->message != '')
    <div class="w-100 d-inline-block">
      <!--My changes style="font-size: 1.1rem"-->
      <div style="font-size: 1.1rem" class="w-auto position-relative text-word-break message bg-light float-left rounded-top-left-0">
        {!! $chatMessage !!}
      </div>
  </div>
@endif
