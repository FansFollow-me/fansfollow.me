<header>
	...
	<nav class="navbar navbar-expand-lg navbar-inverse fixed-top p-nav @if(auth()->guest() && request()->path() == '/') scroll @else p-3 @if (request()->is('live/*')) d-none @endif  @if (request()->is('messages/*')) d-none d-lg-block shadow-sm @elseif(request()->is('messages')) shadow-sm @else shadow-custom @endif {{ auth()->check() && auth()->user()->dark_mode == 'on' ? 'bg-white' : 'navbar_background_color' }} link-scroll @endif">
		<div class="container-fluid d-flex position-relative">
			...
			<a class="navbar-brand col-md-4 mr-5" href="{{url('/')}}">
				@if (auth()->check() && auth()->user()->dark_mode == 'on' )
					<img src="{{url('public/img', $settings->logo)}}" data-logo="{{$settings->logo}}" data-logo-2="{{$settings->logo_2}}" height="43px" width="456px" alt="{{$settings->title}}" class="logo align-bottom max-w-100" />
				@else
				<img src="{{url('public/img', auth()->guest() && request()->path() == '/' ? $settings->logo : $settings->logo_2)}}" data-logo="{{$settings->logo}}" height="43px" width="456px" data-logo-2="{{$settings->logo_2}}" alt="{{$settings->title}}" class="logo align-bottom max-w-100" />
			@endif
			</a>
			...
			@guest
				<!--My Changes-->
				<div class="buttons-mobile-nav d-lg-none">
				<a class="btn-mobile-nav navbar-toggler-mobile text-dark @if(auth()->guest() && request()->path() == '/') text-white @endif" href="#"  data-toggle="collapse" data-target="#navbarCollapse" aria-controls="navbarCollapse" aria-expanded="false" role="button">
					<i class="feather icon-menu icon-navbar"></i>
				</a>
				</div>
			@endguest
			...
			@endif
				...
				<ul class="navbar-nav ml-auto">
					...
					<li class="nav-item">
							<a @if (request()->is('/') && $settings->home_style == 0 || request()->route()->named('profile') || request()->is('creators') || request()->is('creators/*') || request()->is('category/*') || request()->is('p/*') || request()->is('blog') || request()->is('blog/post/*') || request()->is('shop') || request()->is('shop/product/*')) data-toggle="modal" data-target="#loginFormModal" @endif class="toggleRegister nav-link btn btn-main btn-primary pr-3 pl-3 btn-arrow btn-arrow-sm" href="{{ url('signup') }}">
			...
			@else
				...
				<li class="nav-item dropdown d-lg-none mb-2 border-bottom">
					<a href="{{ url('logout') }}" class="nav-link px-2 link-menu-mobile py-1" style="color:#f87171 !important;">
						<div>
			...
			@if (auth()->user()->verified_id == 'yes' || $settings->referral_system == 'on' || auth()->user()->balance != 0.00)
				...
				@if (auth()->user()->verified_id == 'yes')
					<li class="nav-item dropdown d-lg-none border-bottom">
						...
						<div>
							<i class="fas fa-cog mr-2"></i> {{trans('users.settings')}}
						</div>
						</a>
					</li>
				@endif
			...
			@endif
				...
				<li class="nav-item dropdown d-lg-none mt-2 border-top">
					<a href="{{ url('logout') }}" class="nav-link px-2 link-menu-mobile py-1" style="color:#f87171 !important;">
						<div>
							<i class="feather icon-log-out mr-2"></i>
							<span class="d-lg-none">{{ trans('auth.logout') }}</span>
				...
				<!-- =========== End Menu Mobile ============-->
                   ...
                   <li class="nav-item dropdown d-lg-block d-none">
							...
							<a class="nav-link" href="{{url('explore/creators/live')}}"><img width="50px" src="{{Helper::getFile('img/liveIcon.png')}}" />
								@php 
         ...
         ->where('live_streamings.status', '0')->count();
				...
				<li class="nav-item dropdown d-lg-block d-none">
                    ...
                    <!--My Changes-->
                        <a class="dropdown-item dropdown-navbar" href="{{url('settings')}}">
								<i class="fas fa-cog mr-2"></i> {{trans('users.settings')}}
							</a>
				...
				<li class="nav-item">
					<a class="nav-link btn-arrow btn-arrow-sm btn btn-main btn-primary pr-3 pl-3" href="{{url('settings')}}">
						{{ trans('users.edit_profile')}}</a>
				</li>
