...
<!--My chnages-->
	@if ($media->video != '')
	<video data-id="{{$media->id}}" playsinline preload="none" data-type="post" src="{{ Helper::getFile(config('path.videos').$media->video) }}" image="@if($media->thumimge != NULL){{Helper::getFile(config('path.images').$media->thumimge)}}@endif" class="video-js @if (request()->ajax()) video-js-ajax @endif  vjs-fluid" id="player_post_{{$media->id}}" @if ($media->video_poster) poster="{{ Helper::getFile(config('path.videos').$media->video_poster) }}" @endif>
...
@foreach ($mediaImageVideo as $media)
	...
	@endphp
		...
		@if ($media->type == 'image' || $media->type == 'video')
			...
			<a href="{{ $urlMedia }}" class="media-wrapper rounded-0 glightbox lazyloaded" data-src="{{ $videoPoster ?? $urlMedia}}?w=960&h=980" data-gallery="gallery{{$response->id}}" style="">
		    ...
		    @endif
				...
				@if (! $videoPoster)
					<video playsinline muted preload="none" class="video-poster-html">
