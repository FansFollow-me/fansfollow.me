...
<div class="p-3 border-bottom">
  <p class="text-uppercase text-muted mb-1 small font-weight-bold">{{ trans('general.chats') }}</p>
  <h5 class="mb-0">{{ trans('general.messages') }}</h5>
</div>
@endif
...
/* New - Readed */
    ...
    $user = User::where('id', $userID)->where('id', '<>', auth()->user()->id)->firstOrFail();
    
    $to_sub = auth()->user()
      ->mySubscriptionsUser($user->id)
      ->first();
    if($to_sub)
...
<div class="card msg-inbox border-bottom m-0 rounded-0" >
    ...
    {!! Form::open([
						'method' => 'POST',
						'url' => "conversation/delete/$userID",
						'class' => 'message-remove-form'
					]) !!}
    <a data-fromid="{{$userID}}" href="#" data-toid="{{auth()->user()->id}}" class="x-remove-message f-size-20 @if (request()->id == $userID) text-white @endif"><i  class="bi-x"></i></a>
		{!! Form::close() !!}
		<a href="{{url('messages/'.$userID, $username)}}" class="item-chat list-group-item list-group-item-action text-decoration-none p-4{{$styleStatus}}  @if (request()->id == $userID) active disabled @endif" @if (request()->id != $userID) style="background-color:{{$bg}}" @endif>
			<div class="media">
			 ...
			 <div class="media-body overflow-hidden">
				 <div class="d-flex justify-content-between align-items-center">
					<h6 class="media-heading mb-2 text-truncate font-weight-bold">
							 <span style="@if (request()->id != $userID) color: #e2e8f0; @endif">{{$name}}</span>
				       ...
				       @endif
				        ...
				        <small class="text-muted username ml-1">{{'@'.$username}}</small>
					 </h6>
					 
					 
				 </div>

				 <p class="text-truncate @if (request()->id != $userID) text-dark @endif m-0">
					 @if ($messagesCount != 0)
				 ...
				 @endif
					 ...
					 @else
						 {{ $msg->last()->message }}
					 @endif
                    <small class="timeAgo text-truncate mb-2 float-right" data="{{ date('c',strtotime( $msg->last()->created_at ) ) }}"></small>
				 </p>
