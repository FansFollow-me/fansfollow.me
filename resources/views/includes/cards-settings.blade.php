...
<button type="button" class="btn-menu-expand btn btn-primary btn-block mb-2 d-lg-none" type="button" data-toggle="collapse" data-target="#navbarUserHome" aria-controls="navbarUserHome" aria-expanded="false">
    ...
    <small class="text-muted px-4 pt-3 text-uppercase mb-1 font-weight-bold">{{ trans('general.account') }}</small>
				...
				<a href="{{url(auth()->user()->username)}}" class="list-group-item list-group-item-action d-flex justify-content-between url-user">
					...
					<a href="{{url('settings')}}" class="list-group-item list-group-item-action d-flex justify-content-between @if (request()->is('settings')) active @endif">
							<div>
