...
@if ($media->type == 'video')
	...
	<div class="container-media-msg h-auto">
		<video data-id="{{$media->id}}" data-type="post" id="player_message_{{$media->id}}" image="@if($media->thumimge != NULL){{Helper::getFile(config('path.messages').$media->thumimge)}}@endif" src="{{ Helper::getFile(config('path.messages').$media->file) }}" class="video-js @if (request()->ajax()) video-js-ajax @endif  vjs-fluid {{$classInvisible}}" controls preload="auto"  @if ($media->video_poster) poster="{{ Helper::getFile(config('path.messages').$media->video_poster) }}" @endif>
