...
@if ($isStagingDesign)
<div class="mb-4">
  <h5 class="mb-1 text-uppercase text-muted fw-semibold">{{ __('admin.transactions') }}</h5>
  <h2 class="mb-1">Payment ledger</h2>
  <p class="text-muted mb-0">Search payments and refunds from the redesigned finance workspace.</p>
</div>
@endif
<div class="mb-4">
  <h5 class="mb-1 text-uppercase text-muted fw-semibold">{{ __('admin.transactions') }}</h5>
  <h2 class="mb-1">{{ __('admin.transactions') }} ({{$data->total()}})</h2>
  <p class="text-muted mb-0">Search payments, confirm settlement status, and audit creator revenue inside the redesigned admin shell.</p>
</div>
