...
@if ($isStagingDesign)
<div class="mb-4">
  <h5 class="mb-1 text-uppercase text-muted fw-semibold">{{ __('admin.members') }}</h5>
  <h2 class="mb-1">Edit member</h2>
  <p class="text-muted mb-0">Update a creator account from the redesigned admin shell.</p>
</div>
@endif
...
<div class="content">
    ...
    @include('errors.errors-forms')
            ...
            <fieldset class="row mb-3">
              ...
              <label class="col-sm-2 col-form-label text-lg-end">{{ __('Creator Status') }}</label>
              @foreach($creator_status as $status)
              <legend class="col-form-label col-sm-2 pt-0 text-lg-end">{{ ucfirst($status->title) }}</legend>
              <div class="col-sm-1">
                <div class="form-check form-switch form-switch-md">
                 <input class="form-check-input" type="checkbox" name="creator_status[]" 
                 @if (is_array($user_status)) 
                 @foreach($user_status as $st)
                 @if(in_array($status->id,$st))
                 checked="checked" 
                 @endif
                 @endforeach
                 @endif 
                 
                 value="{{$status->id}}" role="switch">
               </div>
              </div>
              @endforeach
            </fieldset><!-- end row -->
