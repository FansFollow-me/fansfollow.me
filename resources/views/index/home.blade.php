...
@section('content')
  @php($isStagingApp = request()->is('app*'))
  <!-- jumbotron -->
    ...
    <div class="container">
      <div class="row">
        <div class="col-lg-5 second">
          ...
          <p>
            <a href="{{ request()->is('app*') ? '/app/creators' : url('creators') }}" class="btn btn-lg btn-main btn-outline-light btn-w-mb px-4 mr-2" role="button">{{trans('general.explore')}}</a>

            <a class="btn btn-lg btn-main btn-outline-light btn-w px-4 toggleRegister btn-arrow" href="{{ $settings->registration_active == '1' ? ($isStagingApp ? '/auth/signup' : url('signup')) : ($isStagingApp ? '/auth/login' : url('login')) }}">
  ...
  <div class="section py-5 py-large">
    <div class="container">
      ...
      <div class="col-12 col-lg-5">
        ...
        <a href="{{ $settings->registration_active == '1' ? ($isStagingApp ? '/auth/signup' : url('signup')) : ($isStagingApp ? '/auth/login' : url('login')) }}" class="btn-arrow btn btn-lg btn-main btn-outline-primary btn-w px-4">
          {{trans('general.getting_started')}}</a>
      </div>
    </div>
    </div><!-- End Container -->
      ...
      <div class="container">
        ...
        @if ($usersTotal > $users->total())
          <div class="w-100 mb-3 text-center">
            <a href="{{ request()->is('app*') ? '/app/creators' : url('creators') }}" class="float-right link-border">{{trans('general.view_all_creators')}} <small class="pl-1"><i class="bi-arrow-right"></i></small></a>
          </div>
        @endif
...
@endif
    ...
    <div class="jumbotron m-0 text-white text-center bg-gradient">
      <div class="container position-relative">
        ...
        <p>
          <a href="{{ request()->is('app*') ? '/app/creators' : url('creators') }}" class="btn btn-lg btn-main btn-outline-light btn-w-mb px-4 mr-2" role="button">{{trans('general.explore')}}</a>
          <a class="btn-arrow btn btn-lg btn-main btn-outline-light btn-w px-4 toggleRegister" href="{{ $settings->registration_active == '1' ? ($isStagingApp ? '/auth/signup' : url('signup')) : ($isStagingApp ? '/auth/login' : url('login')) }}" role="button">
